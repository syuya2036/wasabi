...
type EfiVoid = u8;
type EfiHandle = u64;
type Resule<T> = core::result::Result<T, &'static str>;
...
#[no_mangle]
fn efi_main(_imange_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
...
struct EfiSystemTable {
    _reserved: [u64; 12],
    pub boot_service: &'static EfiBootServicesTable,
}
const _: () = assert!(offset_of!(EfiBootServicesTable, locate_protocol) == 320);
...
) -> Resule<&'a EfiGraphicsOutputProtocol<'a>> {
    let mut graphic_output_protocol = null_mut::<EfiGraphicsOutputProtocol>();
    let status = (efi_system_table.boot_service.locate_protocol)(
